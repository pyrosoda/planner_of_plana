...
# ── 어두운 글자 전용 비교(스킬 레벨용) ─────────────────────
def _extract_dark_text_mask(arr: np.ndarray, out_size: tuple[int, int] = (96, 30)) -> np.ndarray:
    """
    밝은 배경 위의 어두운 텍스트만 추출해서 정규화.
    Lv.1 / M1 같은 작은 UI 숫자 구분에 사용.
    """
    arr = cv2.GaussianBlur(arr, (3, 3), 0)
...
def match_score_textonly(crop: Image.Image, tmpl_path: str) -> float:
    """
    스킬 레벨처럼 텍스트 자체가 핵심인 경우를 위한 보조 점수.
    배경 여백의 영향보다 글자 형태를 더 강하게 본다.
    """
    _, _, tmpl_g = _load_tmpl(tmpl_path)
    if tmpl_g is None:
        return 0.0
    ...
    try:
        ...
        return 0.7 * float(ncc) + 0.3 * float(diff_score)
    except cv2.error:
        return 0.0
...
def read_skill(crop: Image.Image, skill_key: str) -> str:
    ...
    기존 UI 전체 비교 점수 + 텍스트 형태 점수를 함께 사용한다.
    특히 1레벨이 2레벨로 잘못 붙는 문제를 줄이기 위한 로직이다.
    """
    ...
    for lbl, path in cands.items():
        ...
        # locked는 텍스트보다는 전체 외형이 중요하므로 UI 점수만 사용
        final_score = ui_score if lbl == "locked" else (0.55 * ui_score + 0.45 * text_score)
        scores[lbl] = (final_score, ui_score, text_score)

    ranked = sorted(scores.items(), key=lambda x: x[1][0], reverse=True)
    best_lbl, (best_score, best_ui, best_text) = ranked[0]

    # 1 ↔ 2 오인식 보정: 점수가 비슷하면 글자 형태 점수를 더 신뢰
    if len(ranked) >= 2:
        second_lbl, (second_score, second_ui, second_text) = ranked[1]
        pair = {best_lbl, second_lbl}
        if pair == {"1", "2"} and abs(best_score - second_score) <= 0.035:
